...
Finally, the data is stored in a c-like array, where the last index 
changes the fastest.
...
def read(file) -> np.ndarray:
    ...
    for byteorder in ['<','>']:
        ...
        magic = struct.unpack(byteorder + 'i', file[:4])[0]
    ...
    shape = []
    for i in range(rank):
        shape.append(struct.unpack(byteorder + 'i', file[key:key + 4])[0])
        key += 4
    
    #Define a Function to Recursively Fill the Array
    def fill(array, j):
        #If This is the Last Layer, Get the Data
        if len(np.shape(array)) == 1:
            for i in range(len(array)):
                #Get the Data
                array[i] = struct.unpack(code, file[j:j + step])[0]
            ...
            #Recursively Set All the Array Elements
...
def write(array:np.ndarray, filename:str, suffix:str = '.idx', byteorder:str = '@') -> None:
    ...
    file = struct.pack(byteorder + 'i', (D << 8) + len(shape))
    
    #Add the Shape of the Array to the File
    for n in shape:
        file += struct.pack(byteorder + 'i', n)
